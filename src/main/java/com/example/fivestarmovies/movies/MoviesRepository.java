...
public interface MoviesRepository extends CrudRepository<Movies, Integer> {
  ...
  @Query(value = "SELECT * FROM Movies m WHERE m.genre_id = :genre_id ORDER BY m.rating DESC LIMIT :start_limit, 35", nativeQuery = true)
  List<Movies> getMoviesByGenre(@Param("genre_id") Integer genre_id, @Param("start_limit") Integer start_limit);

  @Query(value = "SELECT * FROM Movies m WHERE title LIKE %:title% ORDER BY rating DESC LIMIT :start_limit, 35", nativeQuery = true)
